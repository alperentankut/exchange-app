...
const HomePage = () => {
  ...
  const runJavaScript = `
    (function() {
      ...
      var quarterGold = document.querySelector("#XAUUSD").innerText;
  ...
  const handleMessage = (event) => {
    ...
    setDolarTl(data.dolarTl);
    setEurTl(data.eurTl);
    setGold(data.gold);
    setQuarterGold(data.quarterGold);
    setFetching(false);
  };
  ...
  const saveData = async () => {
    ...
    try {
      ...
      // priceName anahtar olarak kullanılıyor ve değeri price olarak ayarlanıyor
      data[priceName] = price;
      if (data.priceName !== priceName) {
  ...
  return (
    <View style={HomePageStyle.mainContainer}>
      <View style={HomePageStyle.container}>
        <DataContainer dataName={"DOLAR/TL"} data={dolarTl || "Veri yok"} />
        <DataContainer dataName={"EURO/TL"} data={eurTl || "Veri yok"} />
        <DataContainer dataName={"GRAM ALTIN"} data={gold || "Veri yok"} />
        <DataContainer
          dataName={"ÇEYREK ALTIN"}
          data={quarterGold || "Veri yok"}
        />
      ...
      <View style={HomePageStyle.favorites}>
        ...
        {storageData !==undefined && Object.entries(storageData).map(([key, value], index) => (
          <View style={HomePageStyle.listItem} key={key}>
            <ListItem key={key} bottomDivider containerStyle = {{backgroundColor : "#FFF5EE"}}>
              <ListItem.Content
